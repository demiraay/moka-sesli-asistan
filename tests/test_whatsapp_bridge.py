...
class TestWhatsAppBridge(unittest.TestCase):
    ...
    def test_whatsapp_resume_uses_short_natural_context_summary(self):
        self.store.save_user_ai_notes(
            user_id="+905551112233",
            ai_summary="2+1 ile ilgileniyor. Ust butce 8 milyon TL. Alim niyeti var, ihtiyac analizi suruyor.",
            ai_notes={
                "preferred_flat_type_id": "FT-2P1",
                "preferred_flat_type": "2+1",
                "budget_max_try": 8000000,
                "conversation_focus": "qualification",
            },
        )

        fresh_orchestrator = AgentOrchestrator()
        fresh_orchestrator.admin_store = self.store
        fresh_orchestrator.llm_client.generate = MagicMock(side_effect=[
            '{"tool": "answer_general", "args": {"category": "other"}}',
            "Bütçenizi biraz daha netleştirebilir misiniz?"
        ])
        whatsapp_app_module.orchestrator = fresh_orchestrator
        app = whatsapp_app_module.create_app()
        client = app.test_client()

        response = client.post(
            "/whatsapp/message",
            json={"phone_number": "+905551112233", "message": "selam tekrar"},
        )

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertIn("Tekrar merhaba", payload["reply"])
        self.assertIn("2+1 için yaklaşık 8 milyon tl civarında bakıyordunuz", payload["reply"].lower())
        self.assertNotIn("handoff gerekli", payload["reply"].lower())
        self.assertNotIn("kullanici adi", payload["reply"].lower())
    ...
    def test_whatsapp_handoff_sends_location_only_when_user_asks_for_it(self):
        self.store.update_sales_profile(
            {
                "consultant_name": "Ayse Yilmaz",
                "consultant_title": "Satis Danismani",
        ...
        self.orchestrator.llm_client.generate = MagicMock(side_effect=[
            ...
            "Ofis konumunu ve danışmanımızın iletişim bilgilerini hemen paylaşıyorum.",
        ])

        response = self.client.post(
            "/whatsapp/message",
            json={"phone_number": "+905551112233", "message": "adres ve konum atar misin"},
        )

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertTrue(any(item["type"] == "text" for item in payload["follow_up_actions"]))
        self.assertTrue(any(item["type"] == "location" for item in payload["follow_up_actions"]))

    def test_whatsapp_router_driven_location_share_builds_follow_up_actions(self):
        self.store.update_sales_profile(
            {
                "consultant_name": "Ayse Yilmaz",
                "consultant_title": "Satis Danismani",
        ...
        self.orchestrator.llm_client.generate = MagicMock(side_effect=[
            ...
            "Ofisimiz Ümraniye'de; konumu ve iletişim bilgilerini gönderiyorum.",
        ])

        response = self.client.post(
            "/whatsapp/message",
            json={"phone_number": "+905551112233", "message": "ofisiniz nerde"},
        )

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertEqual(payload["router_decision"]["tool"], "trigger_handoff")
        self.assertTrue(payload["context"]["handoff"]["share_location"])
        self.assertTrue(any(item["type"] == "text" for item in payload["follow_up_actions"]))
        self.assertTrue(any(item["type"] == "location" for item in payload["follow_up_actions"]))
